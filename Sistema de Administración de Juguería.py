...
def abrirventana2():
    ...
    def estadisticas():
        ...
        if salida2>0:
            root2.destroy()
            root3=Tk()
            root3.title('Tratamiento Estadístico de Datos')
            root3.geometry('1350x750+0+0')
            root3.config(bg="Gray")
            f1=Frame(root3, width=900, height=650, bd=8, relief="raise")
            f1.pack(side=LEFT)
            f2=Frame(root3, width=440, height=650, bd=8, relief="raise")
            f2.pack(side=RIGHT)
